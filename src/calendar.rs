...
impl CalendarItem {
    pub fn is_reasonable_length(&self) -> bool {
        ...
        Duration::from_secs_f32(duration.as_seconds_f32())
            < Duration::from_hours(2) + Duration::from_mins(30)
    }
...
impl CalendarClient {
    pub async fn new() -> anyhow::Result<Self> {
        ...
        let creds =
            serde_json::from_str::<OAuthCredentials>(&json).context("Couldn't parse token.json")?;
    ...
    pub async fn fetch_current_status(&self) -> anyhow::Result<CalendarInfo> {
        ...
        let events = calendar_response
            .items
            .into_iter()
            .filter(|event| event.is_reasonable_length())
            .collect::<Vec<_>>();
        ...
        for event in events {
            ...
            let supercedes =
                matches!(ret.status, Status::Free) || matches!(event_status, Status::Busy);
...
pub async fn sync_task(tx: watch::Sender<Option<CalendarInfo>>) -> ! {
    ...
    loop {
        match calendar.fetch_current_status().await {
            ...
            Err(e) => {
                if let Some(err) = e.downcast_ref::<FetchError>()
